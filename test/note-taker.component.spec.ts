...
it('should have mat-expansion panel collased by default',()=>{
  let inputelement = fixture.debugElement.query(By.css('mat-expansion-panel'))
  ...
  it('should call addNote() method with right parameters on click of button and success is handled ',()=>{
    ...
    spyOn(notesService,'addNote').and.callThrough();

    let note : Note = new Note();
    note.title = 'Dummy Note';
    note.text = 'Dummy Note Text';

    noteTakerComponent.note = note;
    ...
    expect(notesService.addNote).toHaveBeenCalledWith(note);

    expect(noteTakerComponent.note.title).toBe('');
    expect(noteTakerComponent.note.text).toBe('');
